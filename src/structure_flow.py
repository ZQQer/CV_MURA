...
from .data import Dataset, get_move_points
...
class StructureFlow():
    ...
    def train(self):
        ...
        val_dataset = Dataset(self.config.DATA_VAL_GT, self.config.DATA_VAL_STRUCTURE, self.config)
        ...
        with_classification = self.config.WITHCLASS
        ...
        while(keep_training):
            ...
            for ii, items in enumerate(train_loader):
                ...
                # flow model
                    ...
                    logs = self.flow_model.update_flow(inputs, smooths, gts, maps, self.flow_model.use_correction_loss,
                                                       self.flow_model.use_vgg_loss)
                    iterations = self.flow_model.iterations
                # flow with structure model
                elif model == 3:
                    with torch.no_grad(): 
                        smooth_stage_1 = self.flow_model.structure_forward(inputs, smooths, maps)
                        # print('after stage 1', smooth_stage_1.shape)
                    logs = self.flow_model.update_flow(inputs, smooth_stage_1.detach(), gts, maps,
                                                       self.flow_model.use_correction_loss, self.flow_model.use_vgg_loss,)
                    iterations = self.flow_model.iterations
    ...
    def test(self):
        ...
        model = self.config.MODEL
        print(model, self.config.DATA_TEST_RESULTS)
        ...
        test_dataset = Dataset(self.config.DATA_TEST_GT, self.config.DATA_TEST_STRUCTURE, self.config,
                              mask_file=self.config.DATA_TEST_MASK)#, max_len=120
        ...
        with torch.no_grad(): 
            for items in test_loader:
                ...
                for i in range(outputs_merged.size(0)):
                    name,_ = test_dataset.load_name(index, self.debug)
    ...
    def load_sub(self):
        ...
        index = 0
        ms = self.config.TEST_MASK_SIZE
        mt = self.config.TEST_STEP
        m_edge = self.config.TEST_EDGE
        with torch.no_grad():
            for items in test_loader:
                _, smooths, gts, _ = self.cuda(*items)
                sub_res = np.zeros([test_batch_size, 3, self.config.DATA_TEST_SIZE, self.config.DATA_TEST_SIZE], dtype=np.float)

                for ii in range(-ms + mt+m_edge, self.config.DATA_TEST_SIZE - mt + 1-m_edge, mt):
                    for jj in range(-ms + mt+m_edge, self.config.DATA_TEST_SIZE - mt + 1-m_edge, mt):

                        traverse_mask_npy = get_move_points(gts.shape, ms, ii, jj)
                        traverse_mask = torch.from_numpy(traverse_mask_npy).cuda()
                        inputs = gts*(1-traverse_mask)

                        # structure model
                        if model == 1:
                            outputs = self.flow_model.structure_forward(inputs, smooths, traverse_mask)
                            outputs_merged = (outputs * traverse_mask) + (smooths * (1 - traverse_mask))

                        # flow model
                        elif model == 2:
                            outputs, flow_maps = self.flow_model.flow_forward(inputs, smooths, traverse_mask)
                            outputs_merged = (outputs * traverse_mask) + (gts * (1 - traverse_mask))


                        # inpaint with structure model / joint model
                        else:
                            smooth_stage_1 = self.flow_model.structure_forward(inputs, smooths, traverse_mask)
                            outputs, flow_maps = self.flow_model.flow_forward(inputs, smooth_stage_1, traverse_mask)
                            outputs_merged = (outputs * traverse_mask) + (gts * (1 - traverse_mask))

                        outputs_merged = self.postprocess(outputs_merged)*255.0
                        inputs_show = inputs + traverse_mask
                        normal_gts = self.postprocess(gts) * 255.0

                        for i in range(outputs_merged.size(0)):

                            # tmp = np.array([np.abs(res - gt)*np.tile(m, [3,1,1]) for (res, gt, m) in
                            #                 zip(outputs_merged.cpu().numpy(), gts.cpu().numpy(), traverse_mask_npy)])
                            tmp = np.array(np.abs(outputs_merged.cpu().numpy()[i] - normal_gts.cpu().numpy()[i])*
                                            np.tile(traverse_mask_npy[i], [3,1,1]))

                            name, whole_path = test_dataset.load_name(index*test_batch_size+i, self.debug)
                            pp,ss = whole_path.split('/')[-3],whole_path.split('/')[-2]
                            name = pp+'_'+ss+'_'+name
                            path = os.path.join(self.config.DATA_TEST_RESULTS, name)
                            fname, fext = os.path.splitext(path)
                            # imsave(outputs_merged[i,:,:,:].unsqueeze(0), fname+'_%d_%d'%(ii,jj)+fext)
                            # cv2.imwrite(fname + '_%d_%d-t' % (ii, jj) + fext, np.transpose(tmp, [1,2,0]))

                            sub_res[i] += tmp

                for k in range(gts.size(0)):
                    gts_ = np.transpose(self.postprocess(gts[k, :, :, :].unsqueeze(0)).cpu().numpy()[0] * 255.0, [1,2,0])
                    name, whole_path = test_dataset.load_name(index * test_batch_size + k, self.debug)
                    pp, ss = whole_path.split('/')[-3], whole_path.split('/')[-2]
                    path = os.path.join(self.config.DATA_TEST_RESULTS, pp, ss)
                    dd(path)
                    # fname, fext = os.path.splitext(path)
                    tmp = np.transpose(sub_res[k], [1,2,0])
                    print('.'+os.path.join(path,name))
                    # cv2.imwrite(fname + '_sub.jpg', tmp.astype(np.int))


                    tmp[tmp < 15] = 0.
                    tmp = (tmp / np.max(tmp)) * 255
                    hot = cv2.applyColorMap(np.uint8(tmp), cv2.COLORMAP_JET)  # COLORMAP_PINK COLORMAP_MAGMA
                    out = 0.6 * gts_ + 0.4 * hot
                    cv2.imwrite('.' + os.path.join(path, name).strip('.png') + '_sub.png', tmp.astype(np.int))
                    cv2.imwrite('.'+os.path.join(path,name) , gts_)
                    cv2.imwrite('.'+os.path.join(path,name).strip('.png')+'_hot.png', out)

                print(index * test_batch_size)
                index += 1
        print('\nEnd test....')
