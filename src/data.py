...
class Dataset(data.Dataset):
    def __init__(self, gt_file, structure_file, config, mask_file=None, max_len=1e8):
        self.gt_image_files, self.structure_image_files = [], []
        self.max_len = max_len
        self.config = config
        self.limitation_flag = 0

        for i in range(len(gt_file)):
            self.gt_image_files.extend(self.load_file_list(gt_file[i]))
            self.structure_image_files.extend(self.load_file_list(structure_file[i]))

        print('len(self.gt_image_files)', len(self.gt_image_files), len(self.structure_image_files))
        print(self.gt_image_files[118:121])

        if len(self.gt_image_files) == 0:
            raise(RuntimeError("Found 0 images in the input files " + "\n"))

        if config.MODE == 'test' or config.MODE == 'load_sub':
        ...
        # read masks from files
            ...
            self.mask_dir, _ = os.path.split(self.mask_image_files[0])

    def __getitem__(self, index):
        try:
            item = self.load_item(index)
        except:
            print('loading error: ', index, self.gt_image_files[index])
            item = self.load_item(0)
        return item

    def __len__(self):
        return min(len(self.gt_image_files), self.max_len)
    ...
    def load_item(self, index):
        ...
        gt_image = loader(gt_path, 'RGB')

        structure_image = loader(structure_path, 'L')
    ...
    def load_mask(self, index, img, path):
        ...
        if self.mask_type == 'random_bbox':
            ...
            if self.transform_opt['random_load_mask'] and self.config.MASK_FILE_RANDOM:
                ...
                name = path.split('/')[-1].strip('.png')+'.jpg'
                mask = gray_loader(self.mask_dir + '/' + name)
    ...
    def load_name(self, index, add_mask_name=False):
        ...
        if not add_mask_name:
            return name, self.gt_image_files[index]
        else:
...
def get_move_points(img_shape, mask_size, move_x, move_y):
    batch_size, _, _, IMAGE_SIZE = img_shape
    x1 = max(0, move_x)
    y1 = max(0, move_y)
    x2 = min(IMAGE_SIZE, move_x+mask_size)
    y2 = min(IMAGE_SIZE, move_y+mask_size)
    h = x2-x1
    w = y2-y1
    m = np.zeros((1,IMAGE_SIZE, IMAGE_SIZE), dtype=np.float32)
    m[:,x1:x2, y1:y2] = 1.
    m = np.tile(m, (batch_size,1,1))

    return np.array(m[:, np.newaxis, :, :])
...
def random_ff_mask( config, shape):
    ...
    for i in range(num_v):
        ...
        for j in range(1+np.random.randint(5)):
            ...
            length = 5+np.random.randint(config['ml'])
...
def bbox2mask( bboxs, shape, config):
    ...
    for bbox in bboxs:
        if config['random_size']:
            ...
            h = int(np.random.randint(bbox[2]-config['min_shape'][0])/2)
            w = int(np.random.randint(bbox[3]-config['min_shape'][1])/2)
        else:
            h=0
            w=0
...
def transform_image(transform_param, gt_image, structure_image, normalize=True, toTensor=True):
    transform_list, transform_list_l = [], []
    ...
    if transform_param['crop']:
        ...
        transform_list_l.append(transforms.Lambda(lambda img: __crop(img, crop_position, crop_size)))
    if transform_param['resize']:
        transform_list.append(transforms.Resize(transform_param['resize']))
        transform_list_l.append(transforms.Resize(transform_param['resize']))
    if transform_param['flip']:
        transform_list.append(transforms.Lambda(lambda img: __flip(img, True)))
        transform_list_l.append(transforms.Lambda(lambda img: __flip(img, True)))

    if toTensor:
        transform_list += [transforms.ToTensor()]
        transform_list_l += [transforms.ToTensor()]

    if normalize:
        transform_list_l += [transforms.Normalize((0.5,),
                                                (0.5,))]
        transform_list += [transforms.Normalize((0.5, 0.5, 0.5),
                                                (0.5, 0.5, 0.5))]
    trans = transforms.Compose(transform_list)  
    trans_l = transforms.Compose(transform_list_l)
    ...
    structure_image = trans_l(structure_image)
