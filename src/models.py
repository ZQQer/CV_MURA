...
class StructureFlowModel(BaseModel):
    def __init__(self, config):
        ...
        self.dis_param_s = {'input_dim': 1, 'dim': 64, 'n_layers': 3,
                            'norm': 'none', 'activ': 'lrelu', 'pad_type': 'reflect', 'use_sn': True}
        self.dis_param_f =  {'input_dim':3, 'dim':64, 'n_layers':3,
                         'norm':'none', 'activ':'lrelu', 'pad_type':'reflect', 'use_sn':True}
        ...
        self.add_module('classification_loss', classification_loss)
    ...
    def build_model(self):
        ...
        if self.config.MODEL == 1:
            ...
            self.s_dis = MultiDiscriminator(**self.dis_param_s)
        # flow model with true input smooth
        elif self.config.MODEL == 2:
            self.f_gen = FlowGen(**self.flow_param)
            self.f_dis = MultiDiscriminator(**self.dis_param_f)
    ...
    def structure_forward(self, inputs, smooths, maps):
        if self.config.WITHMEAN_VAR:
            tmp = torch.where(maps[:,1:,:,:]!=0, torch.full_like(maps[:,1:,:,:], 1), maps[:,1:,:,:])
        else:
            tmp = maps
        smooths_input = smooths*(1-tmp)
    ...
    def sample(self, inputs, smooths, gts, maps, labels=None):
        with torch.no_grad():
            ...
            if self.config.MODEL == 1:
                ...
                if maps.shape[1] == 1:
                    ...
                    result = [inputs, smooths, gts, maps[:,0:1,:,:], maps[:,1:,:,:], outputs]
                flow = None
            elif self.config.MODEL == 2:
                ...
                if maps.shape[1] == 1:
                    ...
                    result = [inputs, smooths, gts, maps[:,0:1,:,:], maps[:,1:,:,:], smooth_stage_1, outputs]
                # result =[inputs,smooths,gts,maps,smooth_stage_1,outputs]
                if flow is not None:
                    flow = [flow[:,0,:,:].unsqueeze(1)/30, flow[:,1,:,:].unsqueeze(1)/30]
        return result, flow       

    def update_structure(self, inputs, smooths, maps, class_label = None):
        self.iterations += 1
        ...
        self.structure_adv_dis_loss.backward(retain_graph=True)
        self.s_dis_opt.step()
    ...
    def update_flow(self, inputs, smooths, gts, maps, use_correction_loss, use_vgg_loss, class_label = None):
        self.iterations += 1
        ...
        self.flow_adv_dis_loss.backward(retain_graph=True)
        self.f_dis_opt.step()
