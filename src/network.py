...
class Discriminator(nn.Module):
    def __init__(self, input_dim=3, dim=64, n_layers=3,
                norm='none', activ='lrelu', pad_type='reflect', use_sn=True, with_classific=False, fully_dim=256):
        super(Discriminator, self).__init__()

        self.with_classific = with_classific
        ...
        self.Disout = Conv2dBlock(dim_in,1,1,1,activation='none',use_bias=False, use_sn=use_sn)


        self.pool_c = nn.AvgPool2d(16, stride=16, padding=[1, 1], count_include_pad=False)

        self.fully_c = nn.Linear(fully_dim, 2)


        # self.Classout = Conv2dBlock(dim_in,2,1,1,activation='none',use_bias=False, use_sn=use_sn)
    ...
    def forward(self, x):
        ...
        if self.with_classific:
            n = x.shape[0]
            x = self.pool_c(features)
            x = x.view(n,-1)
            # print(x.shape)
            x = self.fully_c(x)
            # print(x.shape)
            return [self.Disout(features), x]
        else:
            return self.Disout(features)
...
class MultiDiscriminator(nn.Module):
    def __init__(self, **parameter_dic):
        ...
        self.model_1 = Discriminator(**parameter_dic, fully_dim=1024)
...
class StructureGen(nn.Module):
    def __init__(self, input_dim, dim=64, n_res=4, activ='relu',
                 norm='in', pad_type='reflect', use_sn=True, map_depth=1):
        ...
        self.input_layer = Conv2dBlock(input_dim+1+map_depth, dim, 7, 1, 3, norm, activ, pad_type, use_sn=use_sn)
...
class FlowGen(nn.Module):
    def __init__(self, input_dim=3, dim=64, n_res=2, activ='relu',
                norm_flow='ln', norm_conv='in', pad_type='reflect', use_sn=True, map_depth=1):
        super(FlowGen, self).__init__()
                 
        self.flow_column = FlowColumn(input_dim, dim, n_res, activ,
                                      norm_flow, pad_type, use_sn, map_depth)
        self.conv_column = ConvColumn(input_dim, dim, n_res, activ,
                                      norm_conv, pad_type, use_sn, map_depth)
...
class ConvColumn(nn.Module):
    def __init__(self, input_dim=3, dim=64, n_res=2, activ='lrelu',
                 norm='ln', pad_type='reflect', use_sn=True, map_depth=1):
        ...
        self.down_sample += [nn.Sequential(
                            Conv2dBlock(input_dim+1+map_depth, dim//2, 7, 1, 3, norm, activ, pad_type, use_sn=use_sn),
                            Conv2dBlock(dim//2, dim,   4, 2, 1, norm, activ, pad_type, use_sn=use_sn),
    ...
    def forward(self, inputs, flow_maps):   
        ...
        # print('Conv downsample', x1.shape)
        x2 = self.down_sample[1](x1)
        # print('Conv downsample', x2.shape)
        flow_fea = self.resample_image(x1, flow_maps)
        # print('Conv-flow downsample', flow_fea.shape)

        u1 = torch.cat((self.up_sample[0](x2), flow_fea), 1)
        images_out = self.up_sample[1](u1)
        # print('ConvColumn', images_out.shape)
        return images_out
...
class FlowColumn(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # print('Flow downsample shape', f_x4.shape)
        ...
        # print('FlowColumn', flow_map.shape)
        return flow_map
